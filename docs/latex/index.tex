...
The tests/ directory contains the tests for the library, which are written using a home-\/made very basic testing framework. One test exists per source file, and each test must have the same name as the source file it tests. You can run them using the run\+\_\+tests.\+sh script in the main directory.\hypertarget{index_autotoc_md11}{}\doxysubsection{Supported metrics (\+In order of first mention in the paper)}\label{index_autotoc_md11}
