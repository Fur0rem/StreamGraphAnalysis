...
This is only a prototype, so the library is not usable at all for now. I\textquotesingle{}m still experimenting with how I approach the problem for now.\hypertarget{index_autotoc_md3}{}\doxysubsection{How to use the library}\label{index_autotoc_md3}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md10}{}\doxysubsection{Project guidelines and organisation}\label{index_autotoc_md10}
...
The tests/ directory contains the tests for the library, which are written using a home-\/made very basic testing framework. One test exists per source file, and each test must have the same name as the source file it tests. You can run them using the run\+\_\+tests.\+sh script in the main directory.\hypertarget{index_autotoc_md11}{}\doxysubsection{Supported metrics and analysis (\+In order of first mention in the paper)}\label{index_autotoc_md11}
