\doxysection{src/stream.h File Reference}
\hypertarget{stream_8h}{}\label{stream_8h}\index{src/stream.h@{src/stream.h}}


Create Stream\+Graph\textquotesingle{}s and manipulate generic Stream\textquotesingle{}s. A Stream\+Graph is the main data structure to represent a stream graph. It contains all the information about the nodes, links, and its presence in time. ~\newline
 A Stream is a wrapper/interface around a Stream\+Graph. All the functions to access the data and metrics take a Stream as input, and not a Stream\+Graph. The interface asks for the bare minimum to access the data and compute metrics on it. Various actual implementations of Stream\textquotesingle{}s can change the way a Stream\+Graph is viewed. ~\newline
 The 4 main types of Stream are\+:  




\doxysubsection{Detailed Description}
Create Stream\+Graph\textquotesingle{}s and manipulate generic Stream\textquotesingle{}s. A Stream\+Graph is the main data structure to represent a stream graph. It contains all the information about the nodes, links, and its presence in time. ~\newline
 A Stream is a wrapper/interface around a Stream\+Graph. All the functions to access the data and metrics take a Stream as input, and not a Stream\+Graph. The interface asks for the bare minimum to access the data and compute metrics on it. Various actual implementations of Stream\textquotesingle{}s can change the way a Stream\+Graph is viewed. ~\newline
 The 4 main types of Stream are\+: 


\begin{DoxyItemize}
\item Full\+Stream\+Graph\+: A Stream\+Graph as is.
\item Link\+Stream\+: A Stream\+Graph where all nodes are present at all times.
\item Chunk\+Stream and Chunk\+Stream\+Small\+: A subset of the Stream\+Graph.
\item Snapshot\+Stream\+: A Stream\+Graph at a specific time interval. ~\newline
 All of these Stream\textquotesingle{}s use an underlying Stream\+Graph, but transform the data in different ways. ~\newline
 The Stream\textquotesingle{}s do not actually modify the underlying Stream\+Graph, they only provide a different access to it. Each implementation tries to do lazy evaluation and only compute the data when needed, and transform it on the fly. Each implementation can also specialise some implementations of metrics to be more efficient. You can also manipulate multiple Stream\textquotesingle{}s of the same Stream\+Graph at the same time. 
\end{DoxyItemize}{\ttfamily \#include "{}bit\+\_\+array.\+h"{}}\newline
{\ttfamily \#include "{}interval.\+h"{}}\newline
{\ttfamily \#include "{}stream\+\_\+graph/events\+\_\+table.\+h"{}}\newline
{\ttfamily \#include "{}stream\+\_\+graph/key\+\_\+moments\+\_\+table.\+h"{}}\newline
{\ttfamily \#include "{}stream\+\_\+graph/links\+\_\+set.\+h"{}}\newline
{\ttfamily \#include "{}stream\+\_\+graph/nodes\+\_\+set.\+h"{}}\newline
{\ttfamily \#include "{}units.\+h"{}}\newline
{\ttfamily \#include $<$stdbool.\+h$>$}\newline
{\ttfamily \#include $<$stddef.\+h$>$}\newline
{\ttfamily \#include $<$stdint.\+h$>$}\newline
\doxysubsubsection*{Data Structures}
\begin{DoxyCompactItemize}
\item 
struct \mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}}
\begin{DoxyCompactList}\small\item\em The internal implementation of a Stream\+Graph. \end{DoxyCompactList}\item 
struct \mbox{\hyperlink{structOptionalSizeT}{Optional\+SizeT}}
\begin{DoxyCompactList}\small\item\em A size\+\_\+t that can be optional. \end{DoxyCompactList}\item 
struct \mbox{\hyperlink{structInformationCache}{Information\+Cache}}
\begin{DoxyCompactList}\small\item\em A cache of information about the 4 main sets of a Stream\+: the temporal nodes W, the temporal links E, the life span T, and the distinct nodes V. \end{DoxyCompactList}\item 
struct \mbox{\hyperlink{structSGA__Stream}{SGA\+\_\+\+Stream}}
\begin{DoxyCompactList}\small\item\em The structure of a Stream. \end{DoxyCompactList}\end{DoxyCompactItemize}
\doxysubsubsection*{Macros}
\begin{DoxyCompactItemize}
\item 
\#define \mbox{\hyperlink{stream_8h_abb59726803b5ce07a0f602df01da5492}{FETCH\+\_\+\+CACHE}}(stream,  field)
\begin{DoxyCompactList}\small\item\em Fetch a value from the cache of a Stream. \end{DoxyCompactList}\item 
\#define \mbox{\hyperlink{stream_8h_a991491221185567742f92f656f1e4644}{UPDATE\+\_\+\+CACHE}}(stream,  field,  value)
\begin{DoxyCompactList}\small\item\em Write a value to the cache of a Stream. \end{DoxyCompactList}\end{DoxyCompactItemize}
\doxysubsubsection*{Typedefs}
\begin{DoxyCompactItemize}
\item 
typedef struct SGA\+\_\+\+Stream\+Graph \mbox{\hyperlink{stream_8h_a623e7910ee66c5dd739f381b9f77ed24}{SGA\+\_\+\+Stream\+Graph}}
\begin{DoxyCompactList}\small\item\em The structure used to represent a stream graph. \end{DoxyCompactList}\item 
\Hypertarget{stream_8h_a3c628d0526c9c7b163801ccc0e24cb4f}\label{stream_8h_a3c628d0526c9c7b163801ccc0e24cb4f} 
typedef void {\bfseries SGA\+\_\+\+Stream\+Data}
\begin{DoxyCompactList}\small\item\em Blanket struct that serves as a placeholder for the union of all stream types (Full\+Stream\+Graph, Link\+Stream, etc...). Otherwise a circular dependency would be created since these files need \doxylink{stream_8h}{stream.\+h}. It is safe to cast this to the actual type of the Stream (which is known in the Stream struct). \end{DoxyCompactList}\item 
typedef struct SGA\+\_\+\+Stream \mbox{\hyperlink{stream_8h_a6e6f770c3645b9702da2fd1a41a57c06}{SGA\+\_\+\+Stream}}
\begin{DoxyCompactList}\small\item\em The structure of a Stream. \end{DoxyCompactList}\end{DoxyCompactItemize}
\doxysubsubsection*{Functions}
\begin{DoxyCompactItemize}
\item 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \mbox{\hyperlink{stream_8h_a3faf5aebc82385c77d67ad2467cc95e3}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+external\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0}} (const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}format)
\begin{DoxyCompactList}\small\item\em Creates a Stream\+Graph from an external format of the version 1.\+0.\+0 of the library. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \mbox{\hyperlink{stream_8h_a8dea3f09eec5c37beb5b18c267701fd6}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+internal\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0}} (const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}format)
\begin{DoxyCompactList}\small\item\em Creates a Stream\+Graph the internal format of the version 1.\+0.\+0 of the library. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \mbox{\hyperlink{stream_8h_a3da0c73409d0ecabb0743d896492310a}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+string}} (const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}sg\+\_\+as\+\_\+str)
\begin{DoxyCompactList}\small\item\em Creates a Stream\+Graph from a string. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \mbox{\hyperlink{stream_8h_aaf1651fde51147b366ef757df3ae74c7}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+file}} (const char \texorpdfstring{$\ast$}{*}filename)
\begin{DoxyCompactList}\small\item\em Loads a Stream\+Graph from a file. Reads the contents of the file and parses it as a Stream\+Graph. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structString}{String}} \mbox{\hyperlink{stream_8h_a71ed7a37932ee7abca6bfd4d353b14cf}{SGA\+\_\+\+Stream\+Graph\+\_\+to\+\_\+string}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}sg)
\begin{DoxyCompactList}\small\item\em Creates a string representation of a Stream\+Graph. \end{DoxyCompactList}\item 
void \mbox{\hyperlink{stream_8h_a03e621974d5eab4d6946f3b82c9fb89b}{SGA\+\_\+\+Stream\+Graph\+\_\+destroy}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} sg)
\begin{DoxyCompactList}\small\item\em Destroys a Stream\+Graph. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structString}{String}} \mbox{\hyperlink{stream_8h_a4f71af6ced2811fb72efae7a4c9deba6}{SGA\+\_\+external\+\_\+v\+\_\+1\+\_\+0\+\_\+0\+\_\+to\+\_\+internal\+\_\+v\+\_\+1\+\_\+0\+\_\+0\+\_\+format}} (const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}format)
\begin{DoxyCompactList}\small\item\em Converts an external format string to an internal format string. Both from version 1.\+0.\+0. \end{DoxyCompactList}\item 
void \mbox{\hyperlink{stream_8h_aa8e8a255c59a064519663b1ab3a589b6}{init\+\_\+cache}} (\mbox{\hyperlink{structSGA__Stream}{SGA\+\_\+\+Stream}} \texorpdfstring{$\ast$}{*}stream)
\begin{DoxyCompactList}\small\item\em Initialise the cache of a Stream, starting empty. \end{DoxyCompactList}\item 
void \mbox{\hyperlink{stream_8h_a19b523be46204ad6b126b353469c48f4}{reset\+\_\+cache}} (\mbox{\hyperlink{structSGA__Stream}{SGA\+\_\+\+Stream}} \texorpdfstring{$\ast$}{*}stream)
\begin{DoxyCompactList}\small\item\em Reset the cache of a Stream, emptying it. \end{DoxyCompactList}\item 
\mbox{\hyperlink{structSGA__Interval}{SGA\+\_\+\+Interval}} \mbox{\hyperlink{stream_8h_af88ac3f8cd9c51ef6f65bcfb1c66cb70}{SGA\+\_\+\+Stream\+Graph\+\_\+lifespan}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}sg)
\begin{DoxyCompactList}\small\item\em Get the lifespan of a Stream\+Graph. \end{DoxyCompactList}\item 
size\+\_\+t \mbox{\hyperlink{stream_8h_af819fe6f62a1bb1263348ba4df6d9e19}{SGA\+\_\+\+Stream\+Graph\+\_\+time\+\_\+scale}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}sg)
\begin{DoxyCompactList}\small\item\em Get the time scale of a Stream\+Graph. \end{DoxyCompactList}\item 
void \mbox{\hyperlink{stream_8h_aaf759b718d8f8e57d1a3d7b5fd4fbc38}{init\+\_\+events\+\_\+table}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}sg)
\begin{DoxyCompactList}\small\item\em Initialise the events table of a Stream\+Graph. \end{DoxyCompactList}\item 
void \mbox{\hyperlink{stream_8h_af72bc912e3b50f6c91a4c05e6da98f41}{events\+\_\+destroy}} (\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}sg)
\begin{DoxyCompactList}\small\item\em Destroy the events table of a Stream\+Graph. \end{DoxyCompactList}\end{DoxyCompactItemize}


\doxysubsection{Macro Definition Documentation}
\Hypertarget{stream_8h_abb59726803b5ce07a0f602df01da5492}\index{stream.h@{stream.h}!FETCH\_CACHE@{FETCH\_CACHE}}
\index{FETCH\_CACHE@{FETCH\_CACHE}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{FETCH\_CACHE}{FETCH\_CACHE}}
{\footnotesize\ttfamily \label{stream_8h_abb59726803b5ce07a0f602df01da5492} 
\#define FETCH\+\_\+\+CACHE(\begin{DoxyParamCaption}\item[{}]{stream}{, }\item[{}]{field}{}\end{DoxyParamCaption})}

{\bfseries Value\+:}
\begin{DoxyCode}{0}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \textcolor{keywordflow}{if}\ ((stream)-\/>cache.field.present)\ \{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \(\backslash\)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ \ \ return\ (stream)-\/>cache.field.data;\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \(\backslash\)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \}}

\end{DoxyCode}


Fetch a value from the cache of a Stream. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em stream} & The Stream to fetch the value from. \\
\hline
\mbox{\texttt{ in}}  & {\em field} & The field of the cache to fetch. s If the value is present in the cache, it is returned early in the function. Otherwise, the function continues normally. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_a991491221185567742f92f656f1e4644}\index{stream.h@{stream.h}!UPDATE\_CACHE@{UPDATE\_CACHE}}
\index{UPDATE\_CACHE@{UPDATE\_CACHE}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{UPDATE\_CACHE}{UPDATE\_CACHE}}
{\footnotesize\ttfamily \label{stream_8h_a991491221185567742f92f656f1e4644} 
\#define UPDATE\+\_\+\+CACHE(\begin{DoxyParamCaption}\item[{}]{stream}{, }\item[{}]{field}{, }\item[{}]{value}{}\end{DoxyParamCaption})}

{\bfseries Value\+:}
\begin{DoxyCode}{0}
\DoxyCodeLine{\ \ \ \ \ \ \ \ (stream)-\/>cache.field.present\ =\ \textcolor{keyword}{true};\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \(\backslash\)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ (stream)-\/>cache.field.data\ \ \ \ =\ value;}

\end{DoxyCode}


Write a value to the cache of a Stream. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ out}}  & {\em stream} & The Stream to write the value to. \\
\hline
\mbox{\texttt{ out}}  & {\em field} & The field of the cache to write to. \\
\hline
\mbox{\texttt{ in}}  & {\em value} & The value to write to the cache. \\
\hline
\end{DoxyParams}


\doxysubsection{Typedef Documentation}
\Hypertarget{stream_8h_a6e6f770c3645b9702da2fd1a41a57c06}\index{stream.h@{stream.h}!SGA\_Stream@{SGA\_Stream}}
\index{SGA\_Stream@{SGA\_Stream}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_Stream}{SGA\_Stream}}
{\footnotesize\ttfamily \label{stream_8h_a6e6f770c3645b9702da2fd1a41a57c06} 
typedef struct SGA\+\_\+\+Stream SGA\+\_\+\+Stream}



The structure of a Stream. 

Depends on an underlying Stream\+Graph, so its lifetime/scope should be less than the Stream\+Graph. \Hypertarget{stream_8h_a623e7910ee66c5dd739f381b9f77ed24}\index{stream.h@{stream.h}!SGA\_StreamGraph@{SGA\_StreamGraph}}
\index{SGA\_StreamGraph@{SGA\_StreamGraph}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph}{SGA\_StreamGraph}}
{\footnotesize\ttfamily \label{stream_8h_a623e7910ee66c5dd739f381b9f77ed24} 
typedef struct SGA\+\_\+\+Stream\+Graph SGA\+\_\+\+Stream\+Graph}



The structure used to represent a stream graph. 

It is opaque to the user, you should not access its fields directly. 

\doxysubsection{Function Documentation}
\Hypertarget{stream_8h_af72bc912e3b50f6c91a4c05e6da98f41}\index{stream.h@{stream.h}!events\_destroy@{events\_destroy}}
\index{events\_destroy@{events\_destroy}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{events\_destroy()}{events\_destroy()}}
{\footnotesize\ttfamily \label{stream_8h_af72bc912e3b50f6c91a4c05e6da98f41} 
void events\+\_\+destroy (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}}]{sg}{}\end{DoxyParamCaption})}



Destroy the events table of a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ out}}  & {\em sg} & The Stream\+Graph to destroy the events table of. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_aa8e8a255c59a064519663b1ab3a589b6}\index{stream.h@{stream.h}!init\_cache@{init\_cache}}
\index{init\_cache@{init\_cache}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{init\_cache()}{init\_cache()}}
{\footnotesize\ttfamily \label{stream_8h_aa8e8a255c59a064519663b1ab3a589b6} 
void init\+\_\+cache (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__Stream}{SGA\+\_\+\+Stream}} \texorpdfstring{$\ast$}{*}}]{stream}{}\end{DoxyParamCaption})}



Initialise the cache of a Stream, starting empty. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ out}}  & {\em stream} & The Stream to initialise the cache of. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_aaf759b718d8f8e57d1a3d7b5fd4fbc38}\index{stream.h@{stream.h}!init\_events\_table@{init\_events\_table}}
\index{init\_events\_table@{init\_events\_table}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{init\_events\_table()}{init\_events\_table()}}
{\footnotesize\ttfamily \label{stream_8h_aaf759b718d8f8e57d1a3d7b5fd4fbc38} 
void init\+\_\+events\+\_\+table (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}}]{sg}{}\end{DoxyParamCaption})}



Initialise the events table of a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ out}}  & {\em sg} & The Stream\+Graph to initialise the events table of. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_a19b523be46204ad6b126b353469c48f4}\index{stream.h@{stream.h}!reset\_cache@{reset\_cache}}
\index{reset\_cache@{reset\_cache}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{reset\_cache()}{reset\_cache()}}
{\footnotesize\ttfamily \label{stream_8h_a19b523be46204ad6b126b353469c48f4} 
void reset\+\_\+cache (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__Stream}{SGA\+\_\+\+Stream}} \texorpdfstring{$\ast$}{*}}]{stream}{}\end{DoxyParamCaption})}



Reset the cache of a Stream, emptying it. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ out}}  & {\em stream} & The Stream to reset the cache of. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_a4f71af6ced2811fb72efae7a4c9deba6}\index{stream.h@{stream.h}!SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format@{SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format}}
\index{SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format@{SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format()}{SGA\_external\_v\_1\_0\_0\_to\_internal\_v\_1\_0\_0\_format()}}
{\footnotesize\ttfamily \label{stream_8h_a4f71af6ced2811fb72efae7a4c9deba6} 
\mbox{\hyperlink{structString}{String}} SGA\+\_\+external\+\_\+v\+\_\+1\+\_\+0\+\_\+0\+\_\+to\+\_\+internal\+\_\+v\+\_\+1\+\_\+0\+\_\+0\+\_\+format (\begin{DoxyParamCaption}\item[{const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}}]{format}{}\end{DoxyParamCaption})}



Converts an external format string to an internal format string. Both from version 1.\+0.\+0. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em format} & The string in external format to parse. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The string of the internal format 
\end{DoxyReturn}
\Hypertarget{stream_8h_a03e621974d5eab4d6946f3b82c9fb89b}\index{stream.h@{stream.h}!SGA\_StreamGraph\_destroy@{SGA\_StreamGraph\_destroy}}
\index{SGA\_StreamGraph\_destroy@{SGA\_StreamGraph\_destroy}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_destroy()}{SGA\_StreamGraph\_destroy()}}
{\footnotesize\ttfamily \label{stream_8h_a03e621974d5eab4d6946f3b82c9fb89b} 
void SGA\+\_\+\+Stream\+Graph\+\_\+destroy (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}}}]{sg}{}\end{DoxyParamCaption})}



Destroys a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em sg} & The Stream\+Graph to destroy. \\
\hline
\end{DoxyParams}
\Hypertarget{stream_8h_a3faf5aebc82385c77d67ad2467cc95e3}\index{stream.h@{stream.h}!SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0@{SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0}}
\index{SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0@{SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0()}{SGA\_StreamGraph\_from\_external\_format\_v\_1\_0\_0()}}
{\footnotesize\ttfamily \label{stream_8h_a3faf5aebc82385c77d67ad2467cc95e3} 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+external\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0 (\begin{DoxyParamCaption}\item[{const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}}]{format}{}\end{DoxyParamCaption})}



Creates a Stream\+Graph from an external format of the version 1.\+0.\+0 of the library. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em external\+\_\+format} & The string to parse. Assumes it is in the external format. The external format is made to be human-\/readable and writable, but a bit longer to parse. It is composed of only a general section, and the events section. The general header section the lifespan and the time scale. Each event contains the time instant, the type of event, whether it concerns a node or a link, and the node/link concerned. The events are ordered by time. A link can only appear after both its nodes have appeared. Multiple events can happen at the same time. Here\textquotesingle{}s how it looks\+: 
\begin{DoxyCode}{1}
\DoxyCodeLine{SGA\ External\ Format\ version\ 1.0.0}
\DoxyCodeLine{}
\DoxyCodeLine{[General]\ \ \ \ \textcolor{comment}{//\ Header\ of\ the\ general\ section}}
\DoxyCodeLine{Lifespan=(start\ end)}
\DoxyCodeLine{TimeScale=scale}
\DoxyCodeLine{}
\DoxyCodeLine{[\mbox{\hyperlink{structEvents}{Events}}]\ \ \ \ \ \textcolor{comment}{//\ Header\ of\ the\ events\ section}}
\DoxyCodeLine{0\ +\ N\ 1\ \ \textcolor{comment}{//\ Node\ 1\ appears\ at\ time\ 0}}
\DoxyCodeLine{3\ +\ N\ 0\ \ \textcolor{comment}{//\ Node\ 0\ appears\ at\ time\ 3}}
\DoxyCodeLine{3\ +\ L\ 0\ 1\ \ \ \ \textcolor{comment}{//\ Link\ 0-\/1\ appears\ at\ time\ 3}}
\DoxyCodeLine{4\ -\/\ L\ 0\ 1\ \ \ \ \textcolor{comment}{//\ Link\ 0-\/1\ disappears\ at\ time\ 4}}
\DoxyCodeLine{5\ -\/\ N\ 1\ \ \textcolor{comment}{//\ Node\ 1\ disappears\ at\ time\ 5}}
\DoxyCodeLine{6\ -\/\ N\ 0\ \ \textcolor{comment}{//\ Node\ 0\ disappears\ at\ time\ 6}}
\DoxyCodeLine{}
\DoxyCodeLine{[EndOfStream]\ \textcolor{comment}{//\ Signals\ the\ end\ of\ the\ stream}}
\DoxyCodeLine{Any\ text\ after\ \textcolor{keyword}{this}\ is\ ignored,\ and\ can\ be\ used\ \textcolor{keywordflow}{for}\ comments}

\end{DoxyCode}
 You can check the {\ttfamily data/} folder which contains some examples of external formats. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_aaf1651fde51147b366ef757df3ae74c7}\index{stream.h@{stream.h}!SGA\_StreamGraph\_from\_file@{SGA\_StreamGraph\_from\_file}}
\index{SGA\_StreamGraph\_from\_file@{SGA\_StreamGraph\_from\_file}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_from\_file()}{SGA\_StreamGraph\_from\_file()}}
{\footnotesize\ttfamily \label{stream_8h_aaf1651fde51147b366ef757df3ae74c7} 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+file (\begin{DoxyParamCaption}\item[{const char \texorpdfstring{$\ast$}{*}}]{filename}{}\end{DoxyParamCaption})}



Loads a Stream\+Graph from a file. Reads the contents of the file and parses it as a Stream\+Graph. 

\begin{DoxySeeAlso}{See also}
\doxylink{stream_8h_a3da0c73409d0ecabb0743d896492310a}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+string}. 
\end{DoxySeeAlso}

\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em filename} & The name of the file to load. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_a8dea3f09eec5c37beb5b18c267701fd6}\index{stream.h@{stream.h}!SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0@{SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0}}
\index{SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0@{SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0()}{SGA\_StreamGraph\_from\_internal\_format\_v\_1\_0\_0()}}
{\footnotesize\ttfamily \label{stream_8h_a8dea3f09eec5c37beb5b18c267701fd6} 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+internal\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0 (\begin{DoxyParamCaption}\item[{const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}}]{format}{}\end{DoxyParamCaption})}



Creates a Stream\+Graph the internal format of the version 1.\+0.\+0 of the library. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em internal\+\_\+format} & The string to parse. Assumes it is in the internal format. The internal format is made to be more compact and efficient to parse, but way harder to read and write by hand. It is composed of a general section, a memory section, and a data section. The general header section contains the lifespan and the time scale. The memory section contains the number of nodes, links, and key moments. It is also composed of sub-\/sections \+: One for the nodes, which contains its number of neighbours and number of presence intervals. One for the links, which contains its number of presence intervals. One for the key moments, which contains in how many slices the time is divided. \\
\hline
\end{DoxyParams}
\begin{DoxySeeAlso}{See also}
\doxylink{structKeyMomentsTable}{Key\+Moments\+Table}. The data section is also composed of sub-\/sections \+: One for the neighbours, for nodes it contains its neighbouring links, for links it contains the two nodes it links. One for the events, which contains the time of the event, and each events that happened at that time. Unlike the external format, all events of a same time are grouped together. Here\textquotesingle{}s how it looks\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{SGA\ Internal\ version\ 1.0.0}
\DoxyCodeLine{}
\DoxyCodeLine{[General]}
\DoxyCodeLine{Lifespan=(start\ end)}
\DoxyCodeLine{TimeScale=scale}
\DoxyCodeLine{}
\DoxyCodeLine{[Memory]}
\DoxyCodeLine{NumberOfNodes=x}
\DoxyCodeLine{NumberOfLinks=y}
\DoxyCodeLine{NumberOfKeyMoments=z}
\DoxyCodeLine{}
\DoxyCodeLine{[[Nodes]]}
\DoxyCodeLine{[[[NumberOfNeighbours]]]}
\DoxyCodeLine{1\ \ \ \ \textcolor{comment}{//\ Node\ 0\ has\ 1\ neighbour}}
\DoxyCodeLine{2\ \ \ \ \textcolor{comment}{//\ Node\ 1\ has\ 2\ neighbours}}
\DoxyCodeLine{1\ \ \ \ \textcolor{comment}{//\ Node\ 2\ has\ 1\ neighbour}}
\DoxyCodeLine{...}
\DoxyCodeLine{[[[NumberOfIntervals]]]}
\DoxyCodeLine{2\ \ \ \ \textcolor{comment}{//\ Node\ 0\ has\ 2\ presence\ intervals}}
\DoxyCodeLine{1\ \ \ \ \textcolor{comment}{//\ Node\ 1\ has\ 1\ presence\ interval}}
\DoxyCodeLine{3\ \ \ \ \textcolor{comment}{//\ Node\ 2\ has\ 3\ presence\ intervals}}
\DoxyCodeLine{...}
\DoxyCodeLine{}
\DoxyCodeLine{[[Links]]}
\DoxyCodeLine{[[[NumberOfIntervals]]]}
\DoxyCodeLine{1\ \ \ \ \textcolor{comment}{//\ Link\ 0\ has\ 1\ presence\ interval}}
\DoxyCodeLine{2\ \ \ \ \textcolor{comment}{//\ Link\ 1\ has\ 2\ presence\ intervals}}
\DoxyCodeLine{1\ \ \ \ \textcolor{comment}{//\ Link\ 2\ has\ 1\ presence\ interval}}
\DoxyCodeLine{...}
\DoxyCodeLine{}
\DoxyCodeLine{[[[NumberOfKeyMomentsPerSlice]]]}
\DoxyCodeLine{2\ \ \ \ \textcolor{comment}{//\ The\ first\ slice\ contains\ 2\ key\ moments}}
\DoxyCodeLine{...}
\DoxyCodeLine{}
\DoxyCodeLine{[Data]}
\DoxyCodeLine{[[Neighbours]]}
\DoxyCodeLine{[[[NodesToLinks]]]}
\DoxyCodeLine{(1)\ \ \ \ \ \ \textcolor{comment}{//\ Node\ 0\ is\ linked\ to\ link\ 1}}
\DoxyCodeLine{(0\ 2)\ \ \ \ \textcolor{comment}{//\ Node\ 1\ is\ linked\ to\ link\ 0\ and\ 2}}
\DoxyCodeLine{(1)\ \ \ \ \ \ \textcolor{comment}{//\ Node\ 2\ is\ linked\ to\ link\ 1}}
\DoxyCodeLine{...}
\DoxyCodeLine{[[[LinksToNodes]]]}
\DoxyCodeLine{(1\ 3)\ \ \ \ \textcolor{comment}{//\ Link\ 0\ links\ nodes\ 1\ and\ 3}}
\DoxyCodeLine{(0\ 2)\ \ \ \ \textcolor{comment}{//\ Link\ 1\ links\ nodes\ 0\ and\ 2}}
\DoxyCodeLine{(1\ 4)\ \ \ \ \textcolor{comment}{//\ Link\ 2\ links\ nodes\ 1\ and\ 4}}
\DoxyCodeLine{...}
\DoxyCodeLine{[[\mbox{\hyperlink{structEvents}{Events}}]]}
\DoxyCodeLine{0=((+\ N\ 0)\ (+\ N\ 2)\ (+\ L\ 1))\ \ \textcolor{comment}{//\ At\ time\ 0,\ node\ 0\ and\ 2\ appear,\ and\ link\ 1\ appears}}
\DoxyCodeLine{1=((+\ N\ 1)\ (-\/\ L\ 1))\ \ \ \ \ \ \textcolor{comment}{//\ At\ time\ 1,\ node\ 1\ appears,\ and\ link\ 1\ disappears}}
\DoxyCodeLine{...}
\DoxyCodeLine{}
\DoxyCodeLine{[EndOfStream]\ \textcolor{comment}{//\ Signals\ the\ end\ of\ the\ stream}}
\DoxyCodeLine{Any\ text\ after\ \textcolor{keyword}{this}\ is\ ignored,\ and\ can\ be\ used\ \textcolor{keywordflow}{for}\ comments}

\end{DoxyCode}
 
\end{DoxySeeAlso}
\begin{DoxyReturn}{Returns}
The Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_a3da0c73409d0ecabb0743d896492310a}\index{stream.h@{stream.h}!SGA\_StreamGraph\_from\_string@{SGA\_StreamGraph\_from\_string}}
\index{SGA\_StreamGraph\_from\_string@{SGA\_StreamGraph\_from\_string}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_from\_string()}{SGA\_StreamGraph\_from\_string()}}
{\footnotesize\ttfamily \label{stream_8h_a3da0c73409d0ecabb0743d896492310a} 
\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+string (\begin{DoxyParamCaption}\item[{const \mbox{\hyperlink{structString}{String}} \texorpdfstring{$\ast$}{*}}]{sg\+\_\+as\+\_\+str}{}\end{DoxyParamCaption})}



Creates a Stream\+Graph from a string. 


\begin{DoxyParams}{Parameters}
{\em sg\+\_\+as\+\_\+str} & The string to parse. Determines the type and version of the format from the header. \\
\hline
\end{DoxyParams}
\begin{DoxySeeAlso}{See also}
\doxylink{stream_8h_a3faf5aebc82385c77d67ad2467cc95e3}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+external\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0}, 

\doxylink{stream_8h_a8dea3f09eec5c37beb5b18c267701fd6}{SGA\+\_\+\+Stream\+Graph\+\_\+from\+\_\+internal\+\_\+format\+\_\+v\+\_\+1\+\_\+0\+\_\+0}. 
\end{DoxySeeAlso}
\begin{DoxyReturn}{Returns}
The Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_af88ac3f8cd9c51ef6f65bcfb1c66cb70}\index{stream.h@{stream.h}!SGA\_StreamGraph\_lifespan@{SGA\_StreamGraph\_lifespan}}
\index{SGA\_StreamGraph\_lifespan@{SGA\_StreamGraph\_lifespan}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_lifespan()}{SGA\_StreamGraph\_lifespan()}}
{\footnotesize\ttfamily \label{stream_8h_af88ac3f8cd9c51ef6f65bcfb1c66cb70} 
\mbox{\hyperlink{structSGA__Interval}{SGA\+\_\+\+Interval}} SGA\+\_\+\+Stream\+Graph\+\_\+lifespan (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}}]{sg}{}\end{DoxyParamCaption})}



Get the lifespan of a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em sg} & The Stream\+Graph to get the lifespan of. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The lifespan of the Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_af819fe6f62a1bb1263348ba4df6d9e19}\index{stream.h@{stream.h}!SGA\_StreamGraph\_time\_scale@{SGA\_StreamGraph\_time\_scale}}
\index{SGA\_StreamGraph\_time\_scale@{SGA\_StreamGraph\_time\_scale}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_time\_scale()}{SGA\_StreamGraph\_time\_scale()}}
{\footnotesize\ttfamily \label{stream_8h_af819fe6f62a1bb1263348ba4df6d9e19} 
size\+\_\+t SGA\+\_\+\+Stream\+Graph\+\_\+time\+\_\+scale (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}}]{sg}{}\end{DoxyParamCaption})}



Get the time scale of a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em sg} & The Stream\+Graph to get the time scale of. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The time scale of the Stream\+Graph. 
\end{DoxyReturn}
\Hypertarget{stream_8h_a71ed7a37932ee7abca6bfd4d353b14cf}\index{stream.h@{stream.h}!SGA\_StreamGraph\_to\_string@{SGA\_StreamGraph\_to\_string}}
\index{SGA\_StreamGraph\_to\_string@{SGA\_StreamGraph\_to\_string}!stream.h@{stream.h}}
\doxysubsubsection{\texorpdfstring{SGA\_StreamGraph\_to\_string()}{SGA\_StreamGraph\_to\_string()}}
{\footnotesize\ttfamily \label{stream_8h_a71ed7a37932ee7abca6bfd4d353b14cf} 
\mbox{\hyperlink{structString}{String}} SGA\+\_\+\+Stream\+Graph\+\_\+to\+\_\+string (\begin{DoxyParamCaption}\item[{\mbox{\hyperlink{structSGA__StreamGraph}{SGA\+\_\+\+Stream\+Graph}} \texorpdfstring{$\ast$}{*}}]{sg}{}\end{DoxyParamCaption})}



Creates a string representation of a Stream\+Graph. 


\begin{DoxyParams}[1]{Parameters}
\mbox{\texttt{ in}}  & {\em sg} & The Stream\+Graph to convert to a string. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
The string representation of the Stream\+Graph. 
\end{DoxyReturn}
Include dependency graph for stream.\+h\+:\nopagebreak
\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[width=350pt]{stream_8h__incl}
\end{center}
\end{figure}
This graph shows which files directly or indirectly include this file\+:\nopagebreak
\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[width=350pt]{stream_8h__dep__incl}
\end{center}
\end{figure}
