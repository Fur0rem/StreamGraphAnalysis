...
String TreeEdge_to_string(const TreeEdge *edge) {
  char *str = malloc(100);
  sprintf(str, "TreeEdge(weight:%zu, child:%zu)", edge->weight,
          edge->child->id);
...
bool QueueInfo_equals(const QueueInfo *a, const QueueInfo *b) {
  return a->node == b->node && a->time == b->time &&
         ...
         Interval_equals(&a->interval_taken, &b->interval_taken) &&
         a->depth == b->depth;
}
...
String QueueInfo_to_string(const QueueInfo *info) {
  ...
  char buf[100];
  sprintf(buf, "QueueInfo(node:%zu, time:%zu, interval taken:", info->node,
          info->time);
...
String WalkStep_to_string(const WalkStep *step) {
  char *str = MALLOC(100);
  sprintf(str, "WalkStep(link:%zu, time:%zu, interval taken:%s)", step->link,
          step->time, Interval_to_string(&step->interval_taken).data);
  return String_from_owned(str);
}
...
String WalkInfo_to_string(const WalkInfo *wi) {
  if (wi->type == WALK) {
    ...
    if (reason.type == NODE_ABSENT) {
      String interval_str =
          Interval_to_string(&reason.reason.node_absent_between);
...
WalkInfoArrayList optimal_walks_between_two_nodes(
    Stream *stream, NodeId from, NodeId to,
...
WalkStepArrayList WalkStepArrayList_from_candidates(Stream *stream,
                                                    QueueInfo *candidates,
                                                    NodeId from) {
  QueueInfo *current_walk = candidates;
  WalkStepArrayList steps =
      WalkStepArrayList_with_capacity(candidates->previouses);
  StreamFunctions fns = STREAM_FUNCS(fns, stream);
  ...
  while (current_walk != NULL) {
    ...
    LinkId link_id = fns.link_between_nodes(stream->stream_data, current_node,
                                            previous_node);
    if (link_id != SIZE_MAX) {
      WalkStep step = {link_id, current_walk->time,
...
void node_can_still_appear(Stream *stream, NodeId node, TimeId current_time,
                           Interval *current_node_present,
                           Interval *next_appearance, bool *can_still_appear) {
  StreamFunctions fns = STREAM_FUNCS(fns, stream);
  TimesIterator times_node_present =
      fns.times_node_present(stream->stream_data, node);
...
void Stream_shortest_walk_from_to_at_buffered(Stream *stream, NodeId from,
                                              NodeId to, TimeId at,
                                              WalkInfo *result, Arena *arena,
                                              QueueInfoArrayList *queue,
  ...
  QueueInfo start = {from, current_time,
                     .interval_taken = Interval_from(at, max_lifespan),
                     .previous = NULL, .previouses = 0};
  ...
  while ((current_candidate != to) && (!QueueInfoArrayList_is_empty(*queue))) {
    ...
    if (ExploredStateHashset_contains(
            *explored, (ExploredState){current_info.node, current_info.time})) {
      continue;
    }
    ExploredStateHashset_insert(
        explored, (ExploredState){current_info.node, current_info.time});
    ...
    node_can_still_appear(stream, current_candidate, current_time,
                          &current_node_present, &next_appearance,
                          &can_still_appear);
    ...
    if (Interval_is_empty(current_node_present)) {
      *result = node_absent_between(
          Interval_from(current_time, next_appearance.start));
      return;
    }

    LinksIterator neighbours =
        fns.neighbours_of_node(stream->stream_data, current_candidate);
    FOR_EACH_LINK(link_id, neighbours) {
      IntervalArrayList intervals = SGA_collect_times(
          fns.times_link_present(stream->stream_data, link_id));
      for (size_t j = intervals.length;
           j-- > 0;) { // TODO: c'est quoi cette boucle de merde ??????
        ...
        if (can_cross_now || will_cross_later) {
          ...
          NodeId neighbor_id = link.nodes[0] == current_candidate
                                   ? link.nodes[1]
                                   : link.nodes[0];
          ...
          *previous = current_info;
          ...
          QueueInfo neighbor_info = {
              neighbor_id, time_crossed, .interval_taken = interval,
          ...
          for (size_t i = 0; i < queue->length; i++) {
            ...
            if (info->node == neighbor_info.node &&
                info->time ==
                    neighbor_info
                        .time && // TODO: verify if we can improve the
...
// Minimal number of hops between two nodes
WalkInfo Stream_shortest_walk_from_to_at(Stream *stream, NodeId from, NodeId to,
                                         TimeId at) {
  WalkInfo result;
  ...
  Stream_shortest_walk_from_to_at_buffered(stream, from, to, at, &result,
...
WalkInfo Stream_fastest_shortest_walk(Stream *stream, NodeId from, NodeId to,
                                      TimeId at) {
  WalkInfo result;
  StreamFunctions fns = STREAM_FUNCS(fns, stream);
  size_t current_time = at;
  ...
  ExploredStateHashset explored = ExploredStateHashset_with_capacity(50);
  while ((!QueueInfoArrayList_is_empty(queue)) &&
         ((!found) || (current_info.depth <= depth_found))) {
    // Get the next node from the queue
    current_info = QueueInfoArrayList_pop_first(&queue);
    if (ExploredStateHashset_contains(
            explored, (ExploredState){current_info.node, current_info.time})) {
    ...
    FOR_EACH_LINK(link_id, neighbours) {
      Link link = fns.link_by_id(stream->stream_data, link_id);
      TimesIterator times_link_present =
          fns.times_link_present(stream->stream_data, link_id);
...
void Walk_destroy(Walk walk) { WalkStepArrayList_destroy(walk.steps); }
...
String DijkstraState_to_string(DijkstraState *state) {
  ...
  sprintf(str,
          ...
          "interval_taken:%s)",
          state->node, state->time, state->number_of_jumps,
...
WalkInfo Stream_fastest_walk(Stream *stream, NodeId from, NodeId to,
                             TimeId at) {
  ASSERT(from != to);
  ...
  while ((!DijkstraStateBinaryHeap_is_empty(&queue)) &&
         (current_candidate != to)) {
  ...
  while (current_walk != NULL) {
    ...
    FOR_EACH_LINK(i, links) {
      ...
      if ((link.nodes[0] == current_walk->node &&
           link.nodes[1] == previous_node) ||
          (link.nodes[1] == current_walk->node &&
           link.nodes[0] == previous_node)) {
...
size_t Walk_length(Walk *walk) { return walk->steps.length; }
...
double Walk_duration_integral(Interval waiting_period,
                              Interval instantaneous_perioud,
                              TimeId reached_at) {
...
double Walk_duration_integral_1_over_x(Walk *walk) {
  ...
  return Walk_duration_integral(
      Interval_from(walk->optimality.start, stopped_waiting),
...
bool Walk_involves_node_at_time(Walk *walk, NodeId node, double time) {
  for (size_t i = 0; i < walk->steps.length; i++) {
    ...
    if (link.nodes[0] == node || link.nodes[1] == node) {
      ...
      // }
      if ((double)step.interval_taken.start <= time &&
          time <= (double)step.interval_taken.end) {
        return true;
      }
    }
  }
...
double betweenness_of_node_at_time(Stream *stream, NodeId node, double time) {
  ...
  FOR_EACH_NODE(from, nodes) {
    ...
    FOR_EACH_NODE(to, nodes2) {
      if (from != to) {
        WalkInfoArrayList optimal_walks = optimal_walks_between_two_nodes(
            stream, from, to, Stream_fastest_walk);
        for (size_t i = 0; i < optimal_walks.length; i++) {
          ...
          if (walk.type == WALK) {
            ...
            betweenness += (double)number_of_walks_involving_node /
                           (double)number_of_walks;
          }
        }
...
double Walk_length_integral(Walk *walk) { return (double)Walk_length(walk); }
...
double Stream_robustness_by_length(Stream *stream) {
  ...
  NodeIdArrayList nodes =
      SGA_collect_node_ids(fns.nodes_set(stream->stream_data));
  ...
  for (size_t f = 0; f < nodes.length; f++) {
    ...
    for (size_t t = 0; t < nodes.length; t++) {
      ...
      if (from == to) {
        TimesIterator presence =
            fns.times_node_present(stream->stream_data, from);
  ...
  return robustness /
         (double)(nodes.length * nodes.length * (max_lifespan - begin));
}
      ...
      if (from == to) {
        ...
        while (current_time != max_lifespan) {
          ...
          if (optimal.type == NO_WALK) {
            ...
            current_time = optimality.end;
            robustness +=
                (double)(Interval_duration(optimality)) /
...
bool Walk_goes_through(Walk *walk, Stream stream, size_t nb_steps, ...) {
  ...
  Link first_link =
      fns.link_by_id(stream.stream_data, walk->steps.array[0].link);
  ...
  Link last_link =
      fns.link_by_id(stream.stream_data, walk->steps.array[nb_steps - 1].link);
...
Walk WalkInfo_unwrap_unchecked(WalkInfo info) { return info.result.walk; }
