...
NodesIterator ChunkStreamSmall_nodes_set(SGA_StreamData* stream_data) {
...
LinksIterator ChunkStreamSmall_links_set(SGA_StreamData* chunk_stream) {
...
Interval ChunkStreamSmall_lifespan(SGA_StreamData* stream_data) {
	ChunkStreamSmall* chunk_stream = (ChunkStreamSmall*)stream_data;
...
NodesIterator ChunkStreamSmall_nodes_present_at_t(SGA_StreamData* stream_data, TimeId instant) {
	ChunkStreamSmall* chunk_stream		    = (ChunkStreamSmall*)stream_data;
...
LinksIterator ChunkStreamSmall_links_present_at_t(SGA_StreamData* stream_data, TimeId instant) {
	ChunkStreamSmall* chunk_stream		    = (ChunkStreamSmall*)stream_data;
	SGA_Stream st				    = SGA_FullStreamGraph_from(chunk_stream->underlying_stream_graph);
	FullStreamGraph* fsg			    = (FullStreamGraph*)st.stream_data;
...
LinksIterator ChunkStreamSmall_neighbours_of_node(SGA_StreamData* stream_data, NodeId node) {
	ChunkStreamSmall* chunk_stream			= (ChunkStreamSmall*)stream_data;
...
TimesIterator ChunkStreamSmall_times_node_present(SGA_StreamData* stream_data, NodeId node) {
	ChunkStreamSmall* chunk_stream			= (ChunkStreamSmall*)stream_data;
...
TimesIterator ChunkStreamSmall_times_link_present(SGA_StreamData* stream_data, LinkId link) {
	ChunkStreamSmall* chunk_stream			= (ChunkStreamSmall*)stream_data;
	CSS_TimesIdPresentAtIteratorData* iterator_data = MALLOC(sizeof(CSS_TimesIdPresentAtIteratorData));
	size_t nb_skips					= 0;
...
Link ChunkStreamSmall_link_by_id(SGA_StreamData* stream_data, size_t link_id) {
	ChunkStreamSmall* chunk_stream = (ChunkStreamSmall*)stream_data;
	return chunk_stream->underlying_stream_graph->links.links[link_id];
}

// OPTIMISE: why lookup through every link
LinkId ChunkStreamSmall_link_between_nodes(SGA_StreamData* stream_data, NodeId node1, NodeId node2) {
	ChunkStreamSmall* chunk_stream = (ChunkStreamSmall*)stream_data;
	SGA_StreamGraph* stream_graph  = chunk_stream->underlying_stream_graph;
	for (size_t i = 0; i < stream_graph->links.nb_links; i++) {
		Link link = stream_graph->links.links[i];
...
TimesIterator ChunkStreamSmall_key_moments(SGA_StreamData* stream_data) {
	ChunkStreamSmall* chunk_stream = (ChunkStreamSmall*)stream_data;
	SGA_StreamGraph* stream_graph  = chunk_stream->underlying_stream_graph;
