...
void SGA_W_Stream_destroy(SGA_W_Stream stream) {
	switch (stream.base.type) {
		case FULL_STREAM_GRAPH: {
			W_FullStreamGraph_destroy(stream);
			break;
		}
		case LINK_STREAM: {
			W_LinkStream_destroy(stream);
			break;
		}
		case CHUNK_STREAM: {
			W_ChunkStream_destroy(stream);
			break;
		}
		case CHUNK_STREAM_SMALL: {
			W_ChunkStreamSmall_destroy(stream);
			break;
		}
		case TIMEFRAME_STREAM: {
			W_TimeFrameStream_destroy(stream);
			break;
		}
		case DELTA_STREAM: {
			W_DeltaStream_destroy(stream);
			break;
		}
	}
}
