...
LinkId SGA_StreamGraph_link_between_nodes(SGA_StreamGraph* stream_graph, NodeId node_id, NodeId other_node_id) {
	...
	Node n1		   = SGA_StreamGraph_node_by_id(stream_graph, node_id);
	Node n2		   = SGA_StreamGraph_node_by_id(stream_graph, other_node_id);
	...
	for (size_t i = 0; i < n1.nb_neighbours; i++) {
		Link link = SGA_StreamGraph_link_by_id(stream_graph, n1.neighbours[i]);
