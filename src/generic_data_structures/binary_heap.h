...
#define DefineBinaryHeap(type)                                                                                         \
	...
	void type##BinaryHeap_heapify(type##BinaryHeap* heap, size_t idx) {                                                \
		...
		if (left_child_idx < heap->array.size && heap->array.array[left_child_idx] < heap->array.array[idx]) {         \
			smallest_idx = left_child_idx;                                                                             \
		}                                                                                                              \
		if (right_child_idx < heap->array.size &&                                                                      \
			heap->array.array[right_child_idx] < heap->array.array[smallest_idx]) {                                    \
	...
	void type##BinaryHeap_insert(type##BinaryHeap* heap, type value) {                                                 \
		...
		while (idx > 0 && heap->array.array[(idx - 1) / 2] > heap->array.array[idx]) {                                 \
