...
size_t* NodesPresentAtT_next_after_disappearence(NodesPresentAtT* nodes_iter) {
	...
	while (nodes_iter->stream_graph->events.events[nodes_iter->current_event].nb_nodes == 0) {
	...
	size_t* return_val;
	if (nodes_iter->current_node == nodes_iter->stream_graph->events.events[nodes_iter->current_event].nb_nodes) {
...
size_t* NodesPresentAtT_next_before_disappearance(NodesPresentAtT* nodes_iter) {
	...
	if (nodes_iter->current_node == nodes_iter->stream_graph->events.events[nodes_iter->current_event].nb_nodes) {
		...
		if (!BitArray_is_one(nodes_iter->stream_graph->events.presence_mask, nodes_iter->current_event - 1)) {
			return NULL;
		}
		nodes_iter->current_event--;
		nodes_iter->current_node = 0;
...
size_t* LinksPresentAtT_next_after_disappearence(LinksPresentAtT* links_iter) {
	...
	while (links_iter->stream_graph->events.events[links_iter->current_event].nb_links == 0) {
	...
	size_t* return_val;
	if (links_iter->current_link == links_iter->stream_graph->events.events[links_iter->current_event].nb_links) {
		// If you're the last event
		if (links_iter->current_event == links_iter->stream_graph->events.nb_events - 1) {
			return NULL;
		}
	...
	return_val = Event_access_nth_link(links_iter->stream_graph->events.events[links_iter->current_event],
...
size_t* LinksPresentAtT_next_before_disappearance(LinksPresentAtT* links_iter) {
	...
	if (links_iter->current_link == links_iter->stream_graph->events.events[links_iter->current_event].nb_links) {
		...
		// If your event has deletion (presence mask bit 0)
		if (!BitArray_is_one(links_iter->stream_graph->events.presence_mask, links_iter->current_event - 1)) {
			return NULL;
		}
		links_iter->current_event--;
		links_iter->current_link = 0;
	...
	while (links_iter->stream_graph->events.events[links_iter->current_event].nb_links == 0) {
		if ((links_iter->current_event == 0) ||
			(!BitArray_is_one(links_iter->stream_graph->events.presence_mask, links_iter->current_event - 1))) {
			return NULL;
		}
		links_iter->current_event--;
		links_iter->current_link = 0;
